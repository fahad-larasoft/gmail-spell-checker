        ...
        for (var i = 0; i < inputs.length; i++) {
            ...
            for (var j = 0; j < ff.length; j++) {
                ...
                if ("string" == typeof Fields) {
                    var td = Fields, tdcase = Fields.replace(/\s\s*$/, ""),
        ...
        ajax: {
            renderIframe: function (postURL) {
                ...
                var i, j, k, t, r, newSuggestions, sug_each_word, Suggestions,
                    arrResult = strHTML.split(livespell.str.chr(5)), command = arrResult[0],
                    vSender = Number(arrResult[1]), oSender = livespell.spellingProviders[vSender],
                ...
                if (livespell.cache.suggestions[vLang] || (livespell.cache.suggestions[vLang] = []), livespell.cache.spell[vLang] || (livespell.cache.spell[vLang] = []), livespell.cache.reason[vLang] || (livespell.cache.reason[vLang] = []), "CTXSPELL" === command) {
                    ...
                    if ("SAVEWORD" === command) {
                        ...
                        var outInput = arrResult[4].split(livespell.str.chr(1)), isValid = "T" == arrResult[5],
                            outSpellingOk = [], outReason = [];
                        ...
                        var doSuggest = arrResult[4].length > 0;
                        Suggestions = arrResult[4].split(livespell.str.chr(1)), t = arrResult[2].split(""), r = arrResult[3].split(livespell.str.chr(1));
                        var outInput = arrResult[5].split(livespell.str.chr(1)), outSpellingOk = [],
                            outSuggestions = [], outReason = [];
    ...
    Array.push || Array.prototype.push || (Array.prototype.push = function () {
        ...
        setProxyHTMLAndMaintainCaretLegacy: function (id, val) {
            ...
            var range = document.selection.createRange(),
                caretNode = document.getElementById("livespell_cursor_hack__" + id), id2 = id, n = function () {
                ...
                if (e && !e.disabled) {
                    ...
                    if (window.getComputedStyle) {
                        ...
                        for (i = 0; i < stylesToCopy.length; i++) {
                            var attr = stylesToCopy[i], attr2 = stylesToSet[i],
